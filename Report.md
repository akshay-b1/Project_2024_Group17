...
```The completed charts reveal a distinct trend: as the number of processes increases, computation time per rank actually decreases due to the workload being distributed across more processors. However, communication overhead grows steadily with the increase in process count, especially for larger input sizes like 2^28. This trend is consistent across all input types—random, sorted, reverse sorted, and sorted with 1% perturbation—showing that although parallelism reduces individual computation times, it introduces additional communication costs. For random data, time metrics are generally balanced, while sorted and reverse sorted data exhibit slightly higher communication times due to the inherent ordering. The partially sorted 1% perturbation input shows intermediate performance, lying between fully random and fully sorted cases. The increasing variance in time per rank with more processes highlights the need for efficient load balancing strategies to minimize the communication overhead. In summary, while adding more processors reduces computation time per rank, the rising communication costs with larger process counts underscore the challenge of achieving optimal scalability in parallel systems.```
## 5. Presentation
